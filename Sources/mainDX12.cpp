                                   ...
                                   void* _context)
{
    ...
    switch (_category)
    {
    case D3D12_MESSAGE_CATEGORY_APPLICATION_DEFINED: categoryStr = L"Application Defined";
        break;
    case D3D12_MESSAGE_CATEGORY_MISCELLANEOUS: categoryStr = L"Miscellaneous";
        break;
    case D3D12_MESSAGE_CATEGORY_INITIALIZATION: categoryStr = L"Initialization";
        break;
    case D3D12_MESSAGE_CATEGORY_CLEANUP: categoryStr = L"Cleanup";
        break;
    case D3D12_MESSAGE_CATEGORY_COMPILATION: categoryStr = L"Compilation";
        break;
    case D3D12_MESSAGE_CATEGORY_STATE_CREATION: categoryStr = L"State Creation";
        break;
    case D3D12_MESSAGE_CATEGORY_STATE_SETTING: categoryStr = L"State Setting";
        break;
    case D3D12_MESSAGE_CATEGORY_STATE_GETTING: categoryStr = L"State Getting";
        break;
    case D3D12_MESSAGE_CATEGORY_RESOURCE_MANIPULATION: categoryStr = L"Resource Manipulation";
        break;
    case D3D12_MESSAGE_CATEGORY_EXECUTION: categoryStr = L"Execution";
        break;
    case D3D12_MESSAGE_CATEGORY_SHADER: categoryStr = L"Shader";
        break;
    default: categoryStr = L"Unknown";
        break;
    }
    ...
    switch (_severity)
    {
    case D3D12_MESSAGE_SEVERITY_CORRUPTION: SA_LOG(_description, AssertFailure, DX12.ValidationLayers, std::move(dets));
        break;
    case D3D12_MESSAGE_SEVERITY_ERROR: SA_LOG(_description, Error, DX12.ValidationLayers, std::move(dets));
        break;
    case D3D12_MESSAGE_SEVERITY_WARNING: SA_LOG(_description, Warning, DX12.ValidationLayers, std::move(dets));
        break;
    ...
    default: SA_LOG(_description, Normal, DX12.ValidationLayers, std::move(dets));
        break;
    }
}
#endif
...
MComPtr<ID3DBlob> CompileShader(std::wstring _path, std::wstring _entry, std::wstring _target,
                       ...
                       D3D12_RESOURCE_STATES _stateAfter)
{
    ...
    const HRESULT hrStag = device->CreateCommittedResource(&heap, D3D12_HEAP_FLAG_NONE, &desc,
...
bool SubmitTextureToGPU(MComPtr<ID3D12Resource> _gpuTexture, const std::vector<SA::Vec2ui>& _extents,
                        uint64_t _totalSize, uint32_t _channelNum, const void* _data)
{
    ...
    for (UINT16 i = 0; i < resDesc.MipLevels; ++i)
    {
        ...
        const D3D12_TEXTURE_COPY_LOCATION dst{
            .pResource = _gpuTexture.Get(), .Type = D3D12_TEXTURE_COPY_TYPE_SUBRESOURCE_INDEX, .SubresourceIndex = i
        };
    ...
    const D3D12_RESOURCE_BARRIER barrier{
        .Type = D3D12_RESOURCE_BARRIER_TYPE_TRANSITION,
        .Transition = {
            .pResource = _gpuTexture.Get(), .Subresource = D3D12_RESOURCE_BARRIER_ALL_SUBRESOURCES,
...
void GenerateMipMapsCPU(SA::Vec2ui _extent, std::vector<char>& _data, uint32_t& _outMipLevels, uint32_t& _outTotalSize,
                        std::vector<SA::Vec2ui>& _outExtents, uint32_t _channelNum, uint32_t _layerNum = 1u)
{
...
MComPtr<ID3D12Resource> spherePosBuffer; // t8
MComPtr<ID3D12Resource> sphereNormBuffer; // t9
MComPtr<ID3D12Resource> sphereTangBuffer; // t10
MComPtr<ID3D12Resource> sphereUVBuffer; // t11
MComPtr<ID3D12Resource> sphereMeshletBuf; // t5
MComPtr<ID3D12Resource> sphereMeshletVertBuf; // t6
MComPtr<ID3D12Resource> sphereMeshletPrimBuf; // t7
...
MComPtr<ID3D12Resource> CreateDefaultBuffer(uint64_t size, const wchar_t* name)
{
    ...
    const HRESULT hr = device->CreateCommittedResource(&heap, D3D12_HEAP_FLAG_NONE, &desc, D3D12_RESOURCE_STATE_COMMON,
                                                       nullptr, IID_PPV_ARGS(&buf));
    if (FAILED(hr))
    {
        SA_LOG((L"CreateDefaultBuffer {%1} failed!", name), Error, DX12, (L"Error code: %1", hr));
        return nullptr;
    }
    buf->SetName(name);
...
int main()
{
    ...
    if (true)
    {
        ...
        // Renderer
        {
            ...
            if (true)
            {
...
#if SA_DEBUG
                {
                    ...
                    if (SUCCEEDED(hrIQ))
                    {
                        ...
                        dxgiInfoQueue->SetBreakOnSeverity(DXGI_DEBUG_ALL, DXGI_INFO_QUEUE_MESSAGE_SEVERITY_WARNING,
                                                          true);
                    }
                }
                ...
                if (FAILED(hrFactory))
                {
                    SA_LOG(L"Create Factory failed!", Error, DX12, (L"Error Code: %1", hrFactory));
                    return EXIT_FAILURE;
                }
            ...
            if (true)
            {
                ...
                const HRESULT hrGPU = factory->EnumAdapterByGpuPreference(
                    0, DXGI_GPU_PREFERENCE_HIGH_PERFORMANCE, IID_PPV_ARGS(&adapter));
                if (FAILED(hrGPU))
                {
                    SA_LOG(L"Adapter not found!", Error, DX12, (L"Error Code: %1", hrGPU));
                    return EXIT_FAILURE;
                }
                ...
                if (FAILED(hrDev))
                {
                    SA_LOG(L"Create Device failed!", Error, DX12, (L"Error Code: %1", hrDev));
                    return EXIT_FAILURE;
                }
                device->SetName(L"Main Device");
                SA_LOG(L"Create Device success.", Info, DX12, device.Get());
                
                // Verify mesh shader feature
                {
                    D3D12_FEATURE_DATA_SHADER_MODEL smQuery{.HighestShaderModel = D3D_SHADER_MODEL_6_5};
                    const HRESULT hrSM = device->CheckFeatureSupport(D3D12_FEATURE_SHADER_MODEL, &smQuery,
                                                                     sizeof(smQuery));
                ...
                // Queue
                {
                    ...
                    if (FAILED(hrQ))
                    {
                        SA_LOG(L"Create Graphics Queue failed!", Error, DX12, (L"Error Code: %1", hrQ));
                        return EXIT_FAILURE;
                    }
                ...
                // Device fence
                {
                    ...
                    if (FAILED(hrFence))
                    {
                        SA_LOG(L"Create Device Fence failed!", Error, DX12, (L"Error Code: %1", hrFence));
                        return EXIT_FAILURE;
                    }
            ...
            if (true)
            {
                ...
                const HRESULT hrSC = factory->CreateSwapChainForHwnd(graphicsQueue.Get(), glfwGetWin32Window(window),
                                                                     &desc, nullptr, nullptr, &sc1);
                if (FAILED(hrSC))
                {
                    SA_LOG(L"Create Swapchain failed!", Error, DX12, (L"Error Code: %1", hrSC));
                    return EXIT_FAILURE;
                }
                sc1.As(&swapchain);
            ...
            if (true)
            {
                ...
                const HRESULT hrCL = device->CreateCommandList(0, D3D12_COMMAND_LIST_TYPE_DIRECT, cmdAllocs[0].Get(),
                                                               nullptr, IID_PPV_ARGS(&cmdList));
                if (FAILED(hrCL))
                {
                    SA_LOG(L"Create Command List failed!", Error, DX12, (L"Error Code: %1", hrCL));
                    return EXIT_FAILURE;
                }
            ...
            if (true)
            {
                // Color RT View Heap /* 0006-I1 */
                {
                    const D3D12_DESCRIPTOR_HEAP_DESC desc{
                        .Type = D3D12_DESCRIPTOR_HEAP_TYPE_RTV, .NumDescriptors = bufferingCount
                    };
                ...
                // Depth texture /* 0005-I1 */
                {
                    ...
                    device->CreateCommittedResource(&heap, D3D12_HEAP_FLAG_NONE, &desc,
                                                    D3D12_RESOURCE_STATE_DEPTH_WRITE, &sceneDepthClearValue,
                                                    IID_PPV_ARGS(&sceneDepthTexture));
                ...
                // Depth DSV Heap /* 0006-I2 */
                {
                    ...
                    device->CreateDepthStencilView(sceneDepthTexture.Get(), nullptr,
            ...
            if (true)
            {
                // Viewport & Scissor
                viewport = {
                    .TopLeftX = 0, .TopLeftY = 0, .Width = float(windowSize.x), .Height = float(windowSize.y),
                ...
                // Lit
                {
                    // Root Signature /* 0008-1-I */
                    {
                        ...
                        const D3D12_STATIC_SAMPLER_DESC sampler{
                            ...
                            .AddressU = D3D12_TEXTURE_ADDRESS_MODE_WRAP, .AddressV = D3D12_TEXTURE_ADDRESS_MODE_WRAP,
                        ...
                        const HRESULT hrRS = device->CreateRootSignature(
                            0, signature->GetBufferPointer(), signature->GetBufferSize(), IID_PPV_ARGS(&litRootSign));
                        if (FAILED(hrRS))
                        {
                            SA_LOG(L"Create Lit RootSignature failed!", Error, DX12, (L"Error Code: %1", hrRS));
                            return EXIT_FAILURE;
                        }
                    ...
                    auto CreateMeshPipeline = [&](ID3DBlob* ms, ID3DBlob* ps, MComPtr<ID3D12PipelineState>& outPSO,
                                                  const wchar_t* name) -> bool
                    {
                        const D3D12_RENDER_TARGET_BLEND_DESC rtBlend{
                            ...
                            .SrcBlendAlpha = D3D12_BLEND_ONE, .DestBlendAlpha = D3D12_BLEND_ZERO,
                        ...
                        pss.rtFormats.data = {
                            {
                                sceneColorFormat, DXGI_FORMAT_UNKNOWN, DXGI_FORMAT_UNKNOWN, DXGI_FORMAT_UNKNOWN,
                        ...
                        if (FAILED(hrPSO))
                        {
                            SA_LOG((L"Create PipelineState {%1} failed!", name), Error, DX12,
                                   (L"Error Code: %1", hrPSO));
                            return false;
                        }
                        outPSO->SetName(name);
                    ...
                    // Pipeline State — Lit
                    {
                        if (!CreateMeshPipeline(litMeshShader.Get(), litPixelShader.Get(), litPipelineState,
                                                L"LitMeshPipeline"))
                            return EXIT_FAILURE;
                    }
                    ...
                    {
                        debugMeshShader = CompileShader(L"Resources/Shaders/HLSL/MeshShaderDebug.hlsl", L"mainMS_Debug",
                                                        L"ms_6_5");
                        if (!debugMeshShader) return EXIT_FAILURE;

                        debugPixelShader = CompileShader(L"Resources/Shaders/HLSL/MeshShaderDebug.hlsl",
                                                         L"mainPS_Debug", L"ps_6_5");
                        if (!debugPixelShader) return EXIT_FAILURE;

                        if (!CreateMeshPipeline(debugMeshShader.Get(), debugPixelShader.Get(), debugPipelineState,
                                                L"DebugMeshletPipeline"))
                            return EXIT_FAILURE;
                    }
                }
            }
            ...
            if (true)
            {
                ...
                // Camera Buffers
                {
                    ...
                    const D3D12_RESOURCE_DESC desc{
                        ...
                        .Format = DXGI_FORMAT_UNKNOWN, .SampleDesc = {.Count = 1},
                        .Layout = D3D12_TEXTURE_LAYOUT_ROW_MAJOR,
                    };
                    for (uint32_t i = 0; i < bufferingCount; ++i)
                    {
                        device->CreateCommittedResource(&heap, D3D12_HEAP_FLAG_NONE, &desc, D3D12_RESOURCE_STATE_COMMON,
                ...
                // Sphere Object Buffer
                {
                    ...
                    if (!SubmitBufferToGPU(sphereObjectBuffer, sizeof(ObjectUBO), &tr,
                ...
                // PointLights Buffer
                {
                    ...
                    std::array<PointLightUBO, pointLightNum> lights{
                        PointLightUBO{
                            .position = {-0.25f, -1.0f, 0.0f}, .intensity = 4.0f, .color = {1.0f, 1.0f, 0.0f},
                            .radius = 3.0f
                        },
                        PointLightUBO{
                            .position = {1.75f, 2.0f, 1.0f}, .intensity = 7.0f, .color = {0.0f, 1.0f, 1.0f},
                            .radius = 4.0f
                        },
                    };

                    if (!SubmitBufferToGPU(pointLightBuffer, size, lights.data(),
                    ...
                    device->CreateShaderResourceView(pointLightBuffer.Get(), &srvDesc,
            ...
            if (true)
            {
...

                {
                    ...
                    // ---- Vertex position buffer -> t8 ----
                    {
                        ...
                        if (!SubmitBufferToGPU(spherePosBuffer, sz, mesh->mVertices,
                    ...
                    // ---- Vertex normal buffer -> t9 ----
                    {
                        ...
                        if (!SubmitBufferToGPU(sphereNormBuffer, sz, mesh->mNormals,
                                               D3D12_RESOURCE_STATE_NON_PIXEL_SHADER_RESOURCE))
                            return EXIT_FAILURE;
                    }
                    ...
                    // ---- Vertex tangent buffer -> t10 ----
                    {
                        ...
                        if (!SubmitBufferToGPU(sphereTangBuffer, sz, mesh->mTangents,
                                               D3D12_RESOURCE_STATE_NON_PIXEL_SHADER_RESOURCE))
                            return EXIT_FAILURE;
                    }
                    ...
                    // ---- Vertex UV buffer -> t11 ----
                    {
                        ...
                        if (!SubmitBufferToGPU(sphereUVBuffer, sz, uvs.data(),
                                               D3D12_RESOURCE_STATE_NON_PIXEL_SHADER_RESOURCE))
                            return EXIT_FAILURE;
                    }

                    // Build meshlets from the index buffer
                    {
                        ...
                        // Upload meshlet descriptor buffer -> t5
                        {
                            ...
                            if (!SubmitBufferToGPU(sphereMeshletBuf, sz, meshlets.data(),
                                                   D3D12_RESOURCE_STATE_NON_PIXEL_SHADER_RESOURCE))
                                return EXIT_FAILURE;
                        }
                        ...
                        // Upload meshlet vertex index buffer -> t6
                        {
                            ...
                            if (!SubmitBufferToGPU(sphereMeshletVertBuf, sz, meshletVerts.data(),
                                                   D3D12_RESOURCE_STATE_NON_PIXEL_SHADER_RESOURCE))
                                return EXIT_FAILURE;
                        }
                        ...
                        // Upload meshlet primitive buffer -> t7
                        {
                            ...
                            if (!SubmitBufferToGPU(sphereMeshletPrimBuf, sz, meshletPrims.data(),
                                                   D3D12_RESOURCE_STATE_NON_PIXEL_SHADER_RESOURCE))
                                return EXIT_FAILURE;
                        }
                    }
                    ...
                    // ---- Create SRVs for mesh data (heap slots 5-11) ----
                    {
                        ...
                        makeSRV(sphereMeshletVertBuf.Get(),
                                UINT(sphereMeshletVertBuf->GetDesc().Width / sizeof(uint32_t)), sizeof(uint32_t));
                        makeSRV(sphereMeshletPrimBuf.Get(),
                                UINT(sphereMeshletPrimBuf->GetDesc().Width / sizeof(MeshletPrimitive)),
                ...
                // PBR Textures
                {
                                                    ...
                                                    MComPtr<ID3D12Resource>& outTex, const wchar_t* name) -> bool
                    {
                        ...
                        if (!raw)
                        {
                            SA_LOG((L"STBI load {%1} failed", path), Error, STB, stbi_failure_reason());
                            return false;
                        }
                        ch = desiredChannels;
                        ...
                        if (FAILED(hr))
                        {
                            SA_LOG((L"Create Texture {%1} failed", name), Error, DX12, (L"Error code: %1", hr));
                            stbi_image_free(raw);
                            return false;
                        }
                        outTex->SetName(name);
                        ...
                        if (!SubmitTextureToGPU(outTex, extents, totalSize, ch, data.data()))
                        {
                            stbi_image_free(raw);
                            return false;
                        }
                    ...
                    if (!loadAndUploadTexture("Resources/Textures/RustedIron2/rustediron2_basecolor.png", 4,
                                              DXGI_FORMAT_R8G8B8A8_UNORM, rustedIron2AlbedoTexture,
                                              L"RustedIron2 Albedo"))
                        return EXIT_FAILURE;
                    if (!loadAndUploadTexture("Resources/Textures/RustedIron2/rustediron2_normal.png", 4,
                                              DXGI_FORMAT_R8G8B8A8_UNORM, rustedIron2NormalTexture,
                                              L"RustedIron2 Normal"))
                        return EXIT_FAILURE;
                    if (!loadAndUploadTexture("Resources/Textures/RustedIron2/rustediron2_metallic.png", 1,
                                              DXGI_FORMAT_R8_UNORM, rustedIron2MetallicTexture,
                                              L"RustedIron2 Metallic"))
                        return EXIT_FAILURE;
                    if (!loadAndUploadTexture("Resources/Textures/RustedIron2/rustediron2_roughness.png", 1,
                                              DXGI_FORMAT_R8_UNORM, rustedIron2RoughnessTexture,
                                              L"RustedIron2 Roughness"))
                        return EXIT_FAILURE;
                }
            }
        ...
        while (!glfwWindowShouldClose(window))
        {
            ...
            if (accumulateTime >= fixedTime)
            {
                ...
                if (glfwGetKey(window, GLFW_KEY_D) == GLFW_PRESS)
                    cameraTr.position += fixedTime * cameraMoveSpeed *
                        cameraTr.Right();
            ...
            // Render
            {
                ...
                auto cameraBuffer = cameraBuffers[swapchainFrameIndex];
                {
                    ...
                    const SA::Mat4f proj = SA::Mat4f::MakePerspective(
                        cameraFOV, float(windowSize.x) / float(windowSize.y), cameraNear, cameraFar);
                ...
                // Record Commands
                {
                    ...
                    // Transition color -> RenderTarget
                    {
                        const D3D12_RESOURCE_BARRIER barrier{
                            .Type = D3D12_RESOURCE_BARRIER_TYPE_TRANSITION,
                            .Transition = {
                                .pResource = sceneColorRT.Get(), .Subresource = D3D12_RESOURCE_BARRIER_ALL_SUBRESOURCES,
                    ...
                    // Bind & Clear RTs
                    {
                        ...
                        rtvHandle.ptr += device->GetDescriptorHandleIncrementSize(D3D12_DESCRIPTOR_HEAP_TYPE_RTV) *
                            swapchainFrameIndex;
                        const D3D12_CPU_DESCRIPTOR_HANDLE dsvHandle = sceneDepthRTViewHeap->
                        ...
                        cmdList->ClearDepthStencilView(dsvHandle, D3D12_CLEAR_FLAG_DEPTH,
                                                       sceneDepthClearValue.DepthStencil.Depth, 0, 0, nullptr);
                    }
                    ...
                    // Mesh Shader Draw
                    {
                        ...
                        const UINT srvStride = device->GetDescriptorHandleIncrementSize(
                            D3D12_DESCRIPTOR_HEAP_TYPE_CBV_SRV_UAV);
                        const D3D12_GPU_DESCRIPTOR_HANDLE gpuBase = pbrSphereSRVHeap->
    ...
    if (true)
    {
        ...
        if (VLayerCallbackCookie)
        {
            ...
            {
                iq->UnregisterMessageCallback(VLayerCallbackCookie);
